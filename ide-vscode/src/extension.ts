...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    context.subscriptions.push(statusBar, vscode.workspace.onDidChangeTextDocument(onChange), vscode.window.onDidChangeActiveTextEditor(onActiveEditorChange), vscode.workspace.onDidSaveTextDocument(() => flushFocus()), vscode.commands.registerCommand("eop.pair", pairCmd), vscode.commands.registerCommand("eop.logout", logoutCmd), vscode.commands.registerCommand("eop.flush", () => flushAll(true)), vscode.commands.registerCommand("eop.openDashboard", openDashboardCmd), vscode.commands.registerCommand("eop.showLog", () => logger.show()), vscode.commands.registerCommand("eop.pause", () => setPaused(true)), vscode.commands.registerCommand("eop.resume", () => setPaused(false)));
...
function flushFocus() {
    if (activeEditorStart === null || !activeLang)
        return;
...
function bucketKey(lang: string, category: Bucket["category"], aiProvider?: string, aiChannel?: string): string {
    return `${lang}::${category}::${aiProvider ?? ""}::${aiChannel ?? ""}`;
}
function addToBucket(lang: string, category: Bucket["category"], patch: Partial<Bucket> & {
    ai_provider?: AIProvider;
    ai_channel?: AIChannel;
}) {
...
async function flushAll(verbose: boolean) {
    if (paused) {
        if (verbose)
            vscode.window.showInformationMessage("Eye of Providence: paused");
        return;
    }
    ...
    if (events.length === 0) {
        if (verbose)
            vscode.window.showInformationMessage("Eye of Providence: nothing to flush");
        return;
    }
    ...
    try {
        ...
        if (verbose)
            vscode.window.showInformationMessage(`EoP: sent ${data.accepted} events`);
        authRequired = false;
        retryAttempt = 0;
        await saveQueue([]);
    }
...
function scheduleRetry() {
    ...
    const delayMs = Math.min(30000 * Math.pow(2, retryAttempt - 1), 60 * 60000);
    if (retryTimer !== null)
...
async function setPaused(next: boolean) {
    ...
    await renderStatus();
    vscode.window.showInformationMessage(next ? "Eye of Providence: paused" : "Eye of Providence: resumed");
}
async function pairCmd() {
    ...
    const pickPromise = vscode.window.showInformationMessage(`EoP pairing code: ${begin.code} (copied). Enter it in dashboard → Settings → Connected devices.`, "Cancel");
    ...
    while (!cancelled && Date.now() - start < expiresMs) {
        ...
        try {
            ...
            if (data.status === "claimed" && data.token && data.user_id) {
                ...
                authRequired = false;
                await renderStatus();
                vscode.window.showInformationMessage(`Eye of Providence: paired as ${data.user_id.slice(0, 8)}…`);
                return;
            }
        }
        catch (err) {
