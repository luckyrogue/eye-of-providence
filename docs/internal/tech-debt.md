...
| Item | Sev | Where | Detail |
|---|---|---|---|
| ~~Docs freshness sweep (2026-05)~~ | — | `docs/*`, `README.md` | **Done:** attribution/data-model/ops/threat-model fixes, alpha terminology, removed `cli-hooks/` + `proto/`, docs prune (`architecture-bc`, `clickhouse-perf` → data-model; `internal/` for tech-debt + publishing). |
...
| Partial API reference only | S2 | `docs/api/openapi.yaml` | Subset of routes; devices/teams/webauthn still undocumented. |
| Threat model gaps | S2 | `docs/threat-model.md` | Passkey/admin STRIDE added; still light on Resend/Dokploy/GDPR export flows. |
| ~~`architecture-bc.md` duplicate~~ | — | removed | Merged into `backend/AGENTS.md` only. |
| README missing quick-start + version | S1 | `README.md` | No "get started in 5 min". No `v0.1.0` badge. No download links. |
...
| TODO: default providers fallback | S1 | `dashboard/src/entities/user/api/{req.ts:92,types.ts:43}` | `/v1/auth/config` shipped; remove hardcoded fallback. |
| ~~Stub admin overview stats~~ | — | `widgets/admin-overview` | **Done** — KPI tiles implemented. |
