// Per-language breakdown: bar = manual vs ai вертикально стэк, top-N.

import type { LangCell } from "./api";

export function Languages({ cells, top = 5 }: { cells: LangCell[]; top?: number }) {
  const byLang = new Map<string, { manual: number; ai: number; refactor: number; other: number; total: number }>();
  for (const c of cells) {
    const v =
      byLang.get(c.lang) ?? { manual: 0, ai: 0, refactor: 0, other: 0, total: 0 };
  ...
  return (
    <div className="space-y-2">
          ...
          <div key={lang} className="space-y-1">
            <div className="flex items-baseline justify-between text-sm">
              ...
              <div className="text-xs text-muted-foreground tabular-nums">
                {v.total.toLocaleString()} chars · {aiPct}% AI · {manualPct}% manual
              </div>
            </div>
            <div className="flex h-2 overflow-hidden rounded-full bg-secondary" style={{ width: `${widthPct}%` }}>
              {v.manual > 0 && (
      ...
      <div className="flex flex-wrap gap-3 pt-2 text-xs text-muted-foreground">
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-blue-500" /> manual</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-purple-500" /> ai</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-amber-500" /> refactor</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-neutral-400" /> other</span>
      </div>
    </div>
  );
}
