...
// validateString — общий helper для string field с min/max длиной.
func validateString(m map[string]json.RawMessage, key string, minLen, maxLen int, required bool, fieldPath string) *SchemaError {
	raw, present := m[key]
	if !present {
		if !required {
			return nil
...
func validateTextOnly(maxLen int) ValidatorFunc {
	return func(raw json.RawMessage) *SchemaError {
		...
		if verr := validateString(m, "text", 1, maxLen, true, "/"); verr != nil {
			return verr
		}
		for k := range m {
			if k != "text" {
...
func validateCTAButton() ValidatorFunc {
	return func(raw json.RawMessage) *SchemaError {
		...
		if verr := validateString(m, "label", 1, 40, true, "/"); verr != nil {
			return verr
		}
		if verr := validateString(m, "href", 1, 500, true, "/"); verr != nil {
			return verr
		}
		var href string
...
func validatePricingTier() ValidatorFunc {
	return func(raw json.RawMessage) *SchemaError {
		...
		if verr := validateString(m, "tagline", 1, 80, true, "/"); verr != nil {
			return verr
		}
...
func validateFAQItems() ValidatorFunc {
	return func(raw json.RawMessage) *SchemaError {
		...
		for i, item := range rawItems {
			...
			if verr := validateString(item, "q", 1, 200, true, prefix); verr != nil {
				return verr
			}
			if verr := validateString(item, "a", 1, 800, true, prefix); verr != nil {
				return verr
			}
