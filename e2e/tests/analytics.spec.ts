// Analytics endpoints — daily trend, language breakdown, heatmap, categories.
// После ingest нескольких events, агрегаты должны возвращать non-empty data.
//
// CH cascading MV propagation может занять секунды — используем polling.
...
test.describe("analytics", () => {
  ...
  test("invalid days falls back to default (no error)", async ({ api }) => {
    const r = await api.fetch<{ days: number }>("/v1/trend?days=99999");
    expect(r.days).toBe(30); // default per handler
  });
});
